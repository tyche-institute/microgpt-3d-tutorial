...
- **Token** — numbriks teisendatud tähemärk, et matemaatika saaks seda süüa; suured mudelid kasutavad sõnatükke, microGPT üksikuid tähemärke. *(01)*
- **Tokeniseerija (tokenizer)** — otsingutabel, mis teisendab tähemärgid tokeniteks ja tagasi. *(01)*
- **Vektoresitus (embedding)** — 16 numbrist koosnev loend (vektor), mis esindab ühte tokenit mudeli sees. *(04)*
...
- **Softmax** — muundur toorskooridest tõenäosusteks: astenda kõik eksponentsiaalselt, siis jaga kogusummaga. *(00, 03, 05)*
...
- **START/STOP vahimärk (sentinel)** — see üks eriline token, mis sisendis tähistab „teksti algust" ja väljundina ennustatuna „teksti lõppu". *(01)*
...
- **ReLU** — süvaõppe lihtsaim kõver: negatiivsetest numbritest saab 0, positiivsed lähevad muutmata läbi. *(02, 04)*
- **RMSNorm** — ümberskaleerimise samm, mis hoiab iga vektori numbrid tervisliku üldsuurusega, et miski ei plahvataks ega haihtuks; normaliseerimine, ei midagi enamat. *(04)*
...
- **Kadu (loss)** — üks number, mis mõõdab, kui valesti mudel eksib; siin `-log p(tõde)` keskmistatuna üle positsioonide, nii et enesekindlad valed vastused valutavad kõige rohkem. *(01, 05)*
