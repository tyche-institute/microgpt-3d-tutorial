...
🌐 English · [Русский](/ru/04-transformer-block) · [Eesti](/et/04-transformer-block)

This is the lesson where the machine stops being a diagram and becomes a
specific, finite object. One number goes in (a token id), thirty-ish
operations happen — every one of them visible below — and 27 numbers come
out. GPT-4 and microGPT differ here only in quantity: more blocks stacked,
wider vectors, more heads. The *shape* of the journey you're about to trace
is the same one a trillion-dollar industry runs billions of times per
second.
...
> **Try this.**
>
> 1. Play the pulse once end to end, then answer without looking: how many
>    times does the data get normalized? How many times does a residual arc
>    rejoin the path?
> 2. Click the **mlp_fc1** station and check its shape: 16 in, 64 out. Then
>    **mlp_fc2**: 64 in, 16 out. The block breathes — expand, think, then
>    compress.
> 3. Find the station whose Python line you could now rewrite from memory.
>    (Candidate: the embedding add — it's one `zip` away from lesson 00's
>    vector addition.)
> 4. Using the parameter table above, estimate what fraction of all ~4,000
>    dials live in the embedding and lm_head tables, versus inside the
>    block itself. Surprised?
